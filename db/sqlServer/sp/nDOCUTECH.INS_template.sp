USE [DOCUTECH_DEV]
GO
/****** Object:  StoredProcedure [nDOCUTECH].[INS_template]    Script Date: 30/10/2019 14:19:55 ******/
SET ANSI_NULLS ON
GO
...
GO
	...
	ALTER PROC [nDOCUTECH].[INS_template]
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='INS_template', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[INS_template] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'INS_template.sp', @g_id_line_num = 142
	...
	EXEC @g_id_return_status = GET_system_key @id_env_key	= @g_id_env_key, @tx_key_name = 'id_template_key', @id_key_value	= @id_template_key OUTPUT, @num_keys = 1

	SELECT @g_tx_err_msg = 'Error generating key for id_template_key'
	IF (@g_id_return_status != 0)
	BEGIN
		RAISERROR (@g_tx_err_msg, 16, 1)
		IF (@g_is_sp_txn = 1)
	...
	END*/
		...
		INSERT INTO nDOCUTECH.T_TEMPLATE
		(
		...
		(
			...
			, ISNULL(@tx_client_name			 	, '?'		) 		
			, ISNULL(@tx_supplier_name 		 		, '?'		)
			, ISNULL(@tx_received_doc_id 		 	, '?'		)
			, ISNULL(@tx_doc_source 			 	, '?'		)	
			, ISNULL(@tx_pdf_type 			 		, '?'		)
			, ISNULL(@is_template_prepared 	 		, 0			)
			, ISNULL(@tx_processor_type			 	, '?'		)
			, ISNULL(@is_send_to_nuxeo			 	, 0			)
			, ISNULL(@is_parsed_successfully 		, 0			)
			, ISNULL(@tx_doc_type 					, '?'		)

		)

		SELECT @id_template_key = SCOPE_IDENTITY();

		SELECT @g_ct_row = @@rowcount, @g_id_error_key = @@error		
		SELECT @g_tx_err_msg = 'Error encountered while inserting -> nDOCUTECH.INS_template' + '@id_template_key [' + CONVERT(varchar, @id_template_key) + '] '
	...
	SELECT @g_tx_err_msg = 'Exiting[INS_template] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'INS_template.sp', @g_id_line_num = 450
