...
/****** Object:  StoredProcedure [nDOCUTECH].[UPD_doc]    Script Date: 9/11/2019 6:11:07 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='UPD_doc', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[UPD_doc] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_doc.sp', @g_id_line_num = 147
	...
	END
		...
		UPDATE	nDOCUTECH.T_DOC
		SET		  id_doc_ver				= id_doc_ver + 1  
				, --  id_doc_ver				= @id_doc_ver + 1
				...
				, supplier_number 				= ISNULL(@supplier_number 		, supplier_number 		)
				, tx_company_address 			= ISNULL(@tx_company_address 	, tx_company_address 	)
				, tx_invoice_to 				= ISNULL(@tx_invoice_to 		, tx_invoice_to 		)
				, tx_deliver_to 				= ISNULL(@tx_deliver_to 		, tx_deliver_to 		)
				, tx_currency 					= ISNULL(@tx_currency 			, tx_currency 			)
				, receipt_number 				= ISNULL(@receipt_number 		, receipt_number 		)
				, document_number 				= ISNULL(@document_number 		, document_number 		)
				, dt_document					= ISNULL(@dt_document			, dt_document			)
				, tx_association_number			= ISNULL(@tx_association_number	, tx_association_number	)
				, recipient_of_invoice 			= ISNULL(@recipient_of_invoice	, recipient_of_invoice	)
				, vat_number 					= ISNULL(@vat_number 			, vat_number 			)
				, invoice_number 				= ISNULL(@invoice_number 		, invoice_number 		)
				, tx_account_number 			= ISNULL(@tx_account_number 	, tx_account_number 	)
				, tx_voucher_number 			= ISNULL(@tx_voucher_number 	, tx_voucher_number 	)
				, tx_fax_number 				= ISNULL(@tx_fax_number 		, tx_fax_number 		)
				, dt_despatch 		 			= ISNULL(@dt_despatch 		 	, dt_despatch 		 	)
				, delivery_note_no 				= ISNULL(@delivery_note_no 		, delivery_note_no 		)
				, tx_order_id 					= ISNULL(@tx_order_id 			, tx_order_id 			)
				, tx_delivery_note 				= ISNULL(@tx_delivery_note 		, tx_delivery_note 		)
				, tx_delivery_details 			= ISNULL(@tx_delivery_details 	, tx_delivery_details 	)
				, flt_net_invoice_total			= ISNULL(@flt_net_invoice_total	, flt_net_invoice_total	)
				, flt_vat 						= ISNULL(@flt_vat 				, flt_vat 				)
				, flt_vat_rate 					= ISNULL(@flt_vat_rate 			, flt_vat_rate 			)
				, flt_vat_payable 				= ISNULL(@flt_vat_payable 		, flt_vat_payable 		)
				, flt_invoice_amount 			= ISNULL(@flt_invoice_amount 	, flt_invoice_amount 	)
				, flt_total_before_vat 			= ISNULL(@flt_total_before_vat	, flt_total_before_vat	)
				, flt_total_amount_due 			= ISNULL(@flt_total_amount_due	, flt_total_amount_due	)
				, tx_payment_details 			= ISNULL(@tx_payment_details 	, tx_payment_details 	)
				, flt_discount 					= ISNULL(@flt_discount 			, flt_discount			)
				

		WHERE	id_doc_key	= @id_doc_key
		AND		id_doc_ver	= @id_doc_ver

		SELECT @g_ct_row = @@rowcount, @g_id_error_key = @@error
		SELECT @g_tx_err_msg = 'Error encountered while updating -> nDOCUTECH.T_DOC ' + '@id_doc_key [' + CONVERT(varchar, @id_doc_key) + '] '
		...
		SELECT @id_doc_ver = @id_doc_ver + 1
	...
	SELECT @g_tx_err_msg = 'Exiting[UPD_doc] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_doc.sp', @g_id_line_num = 386
