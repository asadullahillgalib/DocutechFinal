USE [DOCUTECH_DEV]
GO
/****** Object:  StoredProcedure [nDOCUTECH].[UPD_template]    Script Date: 30/10/2019 14:21:51 ******/
SET ANSI_NULLS ON
GO
...
GO
	...
	ALTER PROC [nDOCUTECH].[UPD_template]
...
BEGIN
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='UPD_template', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[UPD_template] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_template.sp', @g_id_line_num = 147
	...
	END*/
		...
		UPDATE	nDOCUTECH.T_TEMPLATE
		SET		  id_template_ver				= id_template_ver + 1  
				...
				, tx_client_name 				= ISNULL(@tx_client_name				, tx_client_name				)
				, tx_supplier_name 				= ISNULL(@tx_supplier_name				, tx_supplier_name				)
				, tx_received_doc_id 			= ISNULL(@tx_received_doc_id			, tx_received_doc_id			)
				, tx_doc_source 				= ISNULL(@tx_doc_source					, tx_doc_source					)
				, tx_pdf_type 					= ISNULL(@tx_pdf_type					, tx_pdf_type					)
				, is_template_prepared 			= ISNULL(@is_template_prepared			, is_template_prepared			)
				, tx_processor_type				= ISNULL(@tx_processor_type				, tx_processor_type				)
				, is_send_to_nuxeo				= ISNULL(@is_send_to_nuxeo				, is_send_to_nuxeo				)
				, is_parsed_successfully		= ISNULL(@is_parsed_successfully		, is_parsed_successfully		)
				, tx_doc_type					= ISNULL(@tx_doc_type					, tx_doc_type					)


		WHERE	id_template_key	= @id_template_key
		AND		id_template_ver		= @id_template_ver

		SELECT @g_ct_row = @@rowcount, @g_id_error_key = @@error
		SELECT @g_tx_err_msg = 'Error encountered while updating -> nDOCUTECH.UPD_template ' + '@id_template_key [' + CONVERT(varchar, @id_template_key) + '] '
		...
		SELECT @id_template_ver = @id_template_ver + 1
	...
	SELECT @g_tx_err_msg = 'Exiting[UPD_template] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'UPD_template.sp', @g_id_line_num = 386
