...
public class ScannedItextOxGroupInvoiceUploadService extends AbstractItextScannedInvoiceUploadService {
	...
	private static Logger log = LoggerFactory.getLogger(ScannedItextOxGroupInvoiceUploadService.class);
	@Autowired
		...
		if (null != result) {
			...
			if (result.getErrorsNumber() >= 3) {
				...
				try {
					...
					document.setDocumentDate(new SimpleDateFormat("yyyy/MM/dd").parse(valueToDateOrDefault(result, "orderDate")));
				}
