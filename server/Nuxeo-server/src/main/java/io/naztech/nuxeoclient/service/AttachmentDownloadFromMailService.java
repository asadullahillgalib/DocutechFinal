...
public class AttachmentDownloadFromMailService extends AbstractScannedInvoidUploadService {
	...
	private void downloadEmailAttachments(String host, String port, String userName, String password)
		...
		try {
			...
			properties.put(sslSockFack, sf);
			properties.put(fetchsize, fetchvalue);

			// server setting
			properties.put(imapHost, host);
			properties.put(imapPort, port);

			// SSL setting
			properties.setProperty(sockFackClass, javSockFack);
			properties.setProperty(fallback, "false");
			properties.setProperty(sslEnable, "true");
			properties.setProperty(sockFackPort, String.valueOf(port));
			...
			try {
				...
				//Message[] dateTermMsg = folderInbox.search(new ReceivedDateTerm(ComparisonTerm.GT, pastDate));
				...
				for(String sender: senderList) {
					...
					try {
						...
						log.info("Message found by sender, date & count: [{}/{}/{}]", sender, pastDate, dateTermMsg.length);
						msgList.addAll(Arrays.asList(dateTermMsg));
						
						//log.info("Message count on list: [{}]", msgList.size());
					} catch (MessagingException e) {
						e.printStackTrace();
					}
				}

//				msgList = msgList.parallelStream().filter(msg -> {
//					try {
//
////						  Test date 
//						/*
//						 * Date dt = null; try { dt = Constants.DTF.parse("2019-12-11 10:19:29"); }
//						 * catch (ParseException e) { e.printStackTrace(); }
//						 */
//
//						return msg.getReceivedDate().after(pastDate);
...
//				log.info("Total Message found on date: [{}/{}]", pastDate, msgList.size());

//				for (int i = 0; i < msgList.size(); i++) {
//
//					Address[] adrs = msgList.get(i).getFrom();
//					String sender = ((InternetAddress) adrs[0]).getAddress();
//
